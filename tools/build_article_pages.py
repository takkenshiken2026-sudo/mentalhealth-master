...
def list_or_paragraph(text: str) -> str:
    """本文は段落のみ。箇条書きは CSV 側で「・」行＋空行、または HTML 不要の prose で書く。"""
...
def parse_related_links(value: str, by_slug: dict[str, dict[str, str]], rel_path: Path) -> str:
    links: list[str] = []
    ...
    for item in split_semicolon(value):
        ...
        if target in by_slug:
            href = f"{prefix}articles/{html.escape(target)}/"
            ...
            href = f"{prefix}{html.escape(target)}"
            links.append(f'<a class="related-link" href="{href}">{html.escape(apply_vars(label or target))}</a>')
...
def build_article_html(article: dict[str, str], by_slug: dict[str, dict[str, str]]) -> str:
    ...
    related = parse_related_links(article.get("related_links", ""), by_slug, rel_path)
